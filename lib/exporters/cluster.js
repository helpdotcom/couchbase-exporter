...
class ClusterCollector {
  ...
  async collect() {
    ...
    const [stats, query_result] = await Promise.all([
      cb.getCluster()
    , cb.getSingleStat('/pools/default/buckets/tags/stats/query_requests')
    ])
    ...
    this.gauges.get('cluster_balanced').set({cluster}, balanced_value)
...
async function collectClusterMetrics(registry) {
  const collector = new ClusterCollector(registry)
